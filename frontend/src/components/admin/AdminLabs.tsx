...
export const AdminLabs = () => {
  ...
  const fetchItems = async () => {
    try {
      const res = await fetchApi('/api/labs');
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 shadow-xl">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-lg font-semibold text-zinc-50">Interactive Labs CMS</h3>
        <button onClick={() => { setCurrentItem(null); setFormData({ title: '', video_url: '', description: '' }); setIsModalOpen(true); }} className="flex items-center px-4 py-2 bg-zinc-50 hover:bg-zinc-200 text-zinc-950 rounded-lg text-sm font-semibold transition-colors">
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-zinc-950 border border-zinc-850 rounded-lg p-4">
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={() => { setCurrentItem(item); setFormData({ title: item.title, video_url: item.video_url || '', description: item.description }); setIsModalOpen(true); }} className="p-1.5 text-zinc-400 border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 rounded-lg"><Edit2 className="h-3.5 w-3.5" /></button>
                <button onClick={async () => { if(confirm('Delete?')){ await fetchApi(`/api/labs/${item.id}`, { method: 'DELETE' }); fetchItems(); } }} className="p-1.5 text-red-400 border border-zinc-800 bg-zinc-900 hover:bg-zinc-800 rounded-lg"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-xl p-6 shadow-2xl">
            <h2 className="text-lg font-semibold text-zinc-50 mb-4">{currentItem ? 'Edit Lab' : 'Add Lab'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">Video URL</label>
                <input placeholder="Video URL" value={formData.video_url} onChange={e => setFormData({...formData, video_url: e.target.value})} className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-100 focus:outline-none focus:border-zinc-600 focus:ring-1 focus:ring-zinc-600" />
              </div>
              <div>
