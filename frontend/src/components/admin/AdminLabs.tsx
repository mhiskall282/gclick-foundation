  ...
  return (
    <div className="bg-brand-dark-card border border-brand-dark-border rounded-3xl p-8 shadow-2xl">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-display font-bold">Interactive Labs CMS</h3>
        <button onClick={() => { setCurrentItem(null); setFormData({ title: '', video_url: '', description: '' }); setIsModalOpen(true); }} className="flex items-center px-4 py-2 bg-brand-pink text-white rounded-xl text-sm font-bold">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4">
          <div className="bg-brand-dark-card border border-brand-dark-border rounded-2xl w-full max-w-xl p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <input placeholder="Video URL" value={formData.video_url} onChange={e => setFormData({...formData, video_url: e.target.value})} className="w-full bg-brand-dark-obsidian rounded-xl px-4 py-3 border border-brand-dark-border" />
