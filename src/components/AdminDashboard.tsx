      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 pb-6 border-b border-brand-dark-border">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <span className="text-xs uppercase tracking-wider text-brand-emerald font-bold bg-brand-emerald/10 px-3 py-1 rounded-full border border-brand-emerald/20 flex items-center">
                <Database className="h-3 w-3 mr-1.5" />
                Cluster-01
              </span>
              <span className="flex items-center text-xs text-brand-emerald font-semibold">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`p-2.5 bg-brand-dark-card border border-brand-dark-border hover:bg-[#1E1E21] text-gray-400 hover:text-white rounded-xl transition-all ${isRefreshing ? 'animate-spin text-brand-emerald' : ''}`}
        ...
        <div className="flex border-b border-brand-dark-border mb-8 gap-6 text-sm">
          <button
            ...
            className={`pb-4 font-bold border-b-2 transition-all ${activeTab === 'overview' ? 'border-brand-emerald text-brand-emerald' : 'border-transparent text-gray-400 hover:text-white'}`}
          >
            Overview
          </button>
          <button
            onClick={() => setActiveTab('submissions')}
            className={`pb-4 font-bold border-b-2 transition-all ${activeTab === 'submissions' ? 'border-brand-emerald text-brand-emerald' : 'border-transparent text-gray-400 hover:text-white'}`}
          >
            Sponsorship Log
          </button>
          <button
            onClick={() => setActiveTab('api-logs')}
            className={`pb-4 font-bold border-b-2 transition-all ${activeTab === 'api-logs' ? 'border-brand-emerald text-brand-emerald' : 'border-transparent text-gray-400 hover:text-white'}`}
          >
            API Webhooks
          </button>
        </div>
          ...
          <div className="space-y-8">
              ...
              {stats.map((stat) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-[10px] text-brand-emerald font-bold mt-4 uppercase tracking-wider">{stat.change}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="bg-brand-dark-card border border-brand-dark-border rounded-3xl p-8 shadow-2xl">
              <h2 className="text-xl font-display font-bold text-white mb-6 flex items-center">
                <Activity className="h-5 w-5 text-brand-emerald mr-2.5" />
                Live Node Log
              </h2>
              <div className="space-y-4">
                {[
                  ...
                  <div key={index} className="flex justify-between items-center text-xs p-4 bg-brand-dark-obsidian border border-brand-dark-border rounded-xl">
                    ...
                    <span className="text-[10px] font-bold text-brand-emerald bg-brand-emerald/10 px-2.5 py-1 rounded-md border border-brand-emerald/20">
                      {log.status}
                    </span>
                  </div>
                ))}
                ...
                <tbody className="divide-y divide-brand-dark-border/50">
                  {[
                    ...
                    <tr key={i} className="hover:bg-brand-dark-obsidian/30 transition-colors">
                      ...
                      <td className="p-4 text-brand-emerald font-bold">{row.amount}</td>
                      <td className="p-4 text-gray-400 font-mono text-xs">{row.type}</td>
                      <td className="p-4 text-gray-400 text-xs">{row.date}</td>
                      <td className="p-4 pr-6">
                        <span className="text-[10px] font-bold text-brand-emerald bg-brand-emerald/10 border border-brand-emerald/20 px-2 py-0.5 rounded-full">
                          {row.status}
                        </span>
                      </td>
                    </tr>
            ...
            <div className="space-y-4">
              <div className="p-6 bg-brand-dark-obsidian border border-brand-dark-border rounded-2xl space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[10px] font-bold text-brand-emerald bg-brand-emerald/10 border border-brand-emerald/20 px-2 py-0.5 rounded-full">ACTIVE</span>
                </div>
